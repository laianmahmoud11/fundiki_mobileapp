...
export async function uploadUserProfileImage(
  ...
  try {
    ...
    formData.append(
      'key',
      '45d9985bd0d55e29521b8d94c250a9a1'
    );
...
export async function getSavedHotels(): Promise<SavedHotel[]> {
  ...
  try {
    ...
    const hotelIds = snapshot.docs.map(
      d => d.data().hotelId
    );
    ...
    for (const hotelId of hotelIds) {
      ...
      if (hotelSnap.exists()) {
        ...
        hotels.push({
          ...
          savedAt: snapshot.docs.find(
            d => d.data().hotelId === hotelId
          )?.data().savedAt,
        });
      }
    }
...
export async function unsaveHotel(
  hotelId: string
): Promise<void> {
...
export async function isHotelSaved(
  hotelId: string
): Promise<boolean> {
